...
//    int maximumDay = Integer.MAX_VALUE;
    ...
    private void performOnDayClickCallback(Date date, MotionEvent event) {
        if (listener != null) {
            listener.onDayClick(date, event);
        }
    }
...
//                    Log.e("---", "---");
    ...
    void showCurrentMonth() {
        setCalenderToFirstDayOfMonth(calendarWithFirstDayOfMonth, currentCalender.getTime(), 0, 0);
        setCurrentDate(calendarWithFirstDayOfMonth.getTime());
        performMonthScrollCallback();
    }
